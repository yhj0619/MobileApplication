...
//    String 타입의 매개변수를 전달받음, Integer 타입으로 진행상태 정보 표시, Integer 타입의 결과 생성
    class TextAsyncTask extends AsyncTask<String, Integer, Integer> {
      ...
      /*  필수 구현
        ...
        @Override
        protected Integer doInBackground(String... strings) { //hi, hello 두개의 문자열을 받기에 배열로

            Log.d(TAG, " Async start! " + strings[0] + " " + strings[1]);

//            AsyncTask에서 해야할 작업
            int sum = 0;
            for (int i=1; i <= 100; i++) { //0.05초동안 쉬면서 1~100까지 더해주는 작업 실행
                sum += i;
                Log.d(TAG, "value: " + i);
                publishProgress(sum, i); //i값을 전달. sum&i함께 전달.
                try {
                    Thread.sleep(50);
            ...
            return sum;//Integer 결과 값 전달. 우리가 지정한 것.
        }
...
//        AsyncTask 중지 시 호출
        @Override
        protected void onCancelled() { //cancle된 순간 InterruptedException 발생
