...
//        res/values/strings.xml 의 server_url 값을 읽어옴
    ...
    public void onClick(View v) {
        switch(v.getId()) {
            case R.id.btnDownHtml:
                if (!isOnline()) { //네트워크 활성화 되어있는지 체크!!
                    Toast.makeText(MainActivity.this, "네트워크를 사용가능하게 설정해주세요.", Toast.LENGTH_SHORT).show();
                    return;
                }
                String targetDate = etTargetDate.getText().toString(); //날짜 가지고오기
                if (targetDate.equals("")) targetDate = etTargetDate.getHint().toString();  // 입력값이 없을 경우 hint 속성의 값을 기본 값으로 설정
                new NetworkAsyncTask().execute(apiAddress + targetDate);    // 매개변수로 server_url 에 입력한 날짜를 결합한 후 AsyncTask 실행
                                //execute로 실행. url 전달
                break;
        }
    }
    ...
    class NetworkAsyncTask extends AsyncTask<String, Void, String> {//thread역할 클래스
...
//실행 하기 전 수행하는 작업. 대기상태 안내해주는 다이어로그 띄우는 작업
        @Override
        protected void onPreExecute() { //실행 전 수행 준비.
            super.onPreExecute();
            progressDlg = ProgressDialog.show(MainActivity.this, "Wait", "Downloading...");     // 진행상황 다이얼로그 출력
        }

//네트워크 작업을 여기서.Tread 작업!!
        @Override
        protected String doInBackground(String... strings) {//esecute 실행하는 순간 해당메소드 시작됨, 별도의 thread
            String address = strings[0];
            String result = downloadContents(address); //4주차 과제 처럼.
            //downloadContents가 네트워크 작업 메소드

            if (result == null) { //result 없으면
                cancel(true);
...
//doIn~ 이 끝나면 그 결과를 여기로 전달. 이것도 자동실행. 여기 매개변수의 result는 xml 전체 데이터임
       @Override
        protected void onPostExecute(String result) { //doInBack읭 thread 작업 끝나면 결과가 result 매개변수로 자동호출됨
